...
It can also be used to fill areas with an image used as a pattern, almost exactly like the action \textsf{img()} (see Section \ref{sec:img}). The difference between image filling with \textsf{img()} and \textsf{draw()} is that \textsf{draw()} supports \emph{directional} filling (see below) which is useful if an object has holes. Although \emph{directional} filling could always be used instead of normal filling, it is a more complex operation and thus slightly slower.
