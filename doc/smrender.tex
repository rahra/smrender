...
pdfauthor={Bernhard R. Fischer, 4096R/8E24F29D, <bf@abenteuerland.at>},
pdfsubject={},
...
This document describes the current (2025/12/31) version of \smrender{}.
...
order of their \emph{id}. That means that the version is used to group certain rules together.
...
%   \ref{sec:filter}).
   ...
   \item[\optk{B}] \optv{border}
      ...
      add e.g. add a bleed margin for printing. Rendering is always done in respect
   ...
   \item[\optk{b}] \optv{color}
      ...
      This also allows to specify a transparent (e.g. with \textsf{-b transparent}) background which could be of special interest if you create a PNG file as output image.
...
\item[\optk{w}] \optv{file}
   ...
   This option specifies the file name of an OSM output file. \smrender{} will dump all nodes, ways, and relations of its internal in-memory database to this file.

   During processing and rendering, \smrender{} may generate a lot of additional nodes and ways depending on options and the ruleset, in addition to the input data.

   This data will include e.g. the grid but also objects and tags which have been modified or added by the actions found in Section \ref{sec:manipulation}.

   The output file is a valid OSM data file and could be opened in other OSM processing tools such as JOSM.
...
\item[\optk{x}]
   ...
   The index file is created in the same directory as the input file as specified by \optk{i} and its name will have the extension \textsf{.index} added. This location can be changed with option \optk{X}.

\item[\optk{X}]

   This option specifies the name and location of the index file (see option \optk{x}). This may be desirable if e.g. the OSM input file rests on a read only storage.

   If this option is set, option \optk{x} is implied automatically.
...
with tags. Nodes are considered to be rules for rendering nodes, ways are
rules applied to ways, and relations are rules applied to relations. Each object (node,  way, or relation) has a list of tags. These
tags represent patterns which are matched against the tags of the objects which
are to be rendered (as input by option \optk{i}). The values of a tag's key (\textsf{k=``\ldots''}) and/or value
...
below in Section \ref{sec:act}. Of course this tag is not taken into account during the matching process as described previously.

The match algorithm always applies all tags to match, and all of them have to match in order
to execute the action. If just a single tag does not match, the object is skipped.

\paragraph{Example:}

The following rule places a caption with the specifed font at the center of all nodes which have the tags \textsf{scalerank=2} and \textsf{FEATURECL=Admin-1*} where the latter is matched as a regular expression. The caption is based on the value of the tag \textsf{NAME}.

\begin{verbatim}
<node>
   <tag k='FEATURECLA' v='/Admin-1.*/'/>
   <tag k='scalerank' v='2'/>
   <tag k='_action_' v='cap:font=sans-serif:bold;size=1.8;key=NAME'/>
</node>
\end{verbatim}

\subsection{Open and Closed Ways}

Ways are a series of nodes. They can be open like lines or closed like polygons (see also Section \ref{sec:act-draw}). Closed ways are such where the first node in the list of nodes is the same as the last node.

By default way-rules match all ways, independently if they are open or closed but there is an option to tell \smrender{} to match either just closed or open ways. This is done by adding two references to the rule with ids which are either equal or unequal to each other. \smrender{} does not care about the absolute value of the reference ids but just its equality.

\paragraph{Example:}

The following rule writes all closed ways -- and only closed ones -- with the tag \textsf{featurecla=Admin-0} to the file \textsf{admin0.osm}. Closed ways are chosen because there are two references which both share the same id (1 in this case).

\begin{verbatim}
<way>
   <tag k='featurecla' v='Admin-0'/>
   <tag k='_action_' v='out:file=admin0.osm'/>
   <nd ref='1'/>
   <nd ref='1'/>
</way>
\end{verbatim}
...
ignored. Numbers equal or greater to that are reserved to be used for sub-rules. See Section \ref{sec:act-sub} for further details.

The number of iterations, i.e. the number of different versions is
...
Here is a list of all functions currently implemented and built-in into \smrender{}. Please note that there are additional sea-chart-specific functions found in the the shared library \textsf{libsmfilter} which is bundled with \smrender{}. These functions are described in Section \ref{sec:smfilter}.
...
You can use this to add specific OSM objects during the stage of
...
\subsubsection{Adding Tags to Objects} \label{sec:act-set-tags}
...
This object must not have an action tag. The template should have an id because
...
Have a look at \textsf{strfmt} in Section \ref{sec:act-strfmt} which
allows you to dynamically construct and add tags to objects.
...
\subsubsection{Standard Shapes} \label{sec:shape}\label{sec:act-shape}
...
\subsubsection{Create Formatted Strings} \label{sec:strfmt}\label{sec:act-strfmt}
...
If the tag already exists in an object it will be overwritten.
...
\subsubsection{Concatenating Ways} \label{sec:act-cat-poly}

\begin{verbatim}
   <action>   := 'cat_poly:' <param> [ ';' <param> ]
   <param>    := <nocorner> | <incomp> | <copy> | <vcdist>
   <nocorner> := 'no_corner' '=' <bool>
   <incomp>   := 'ign_incomplete' '=' <bool>
   <copy>     := 'copy' '=' <string>
\end{verbatim}

      This function closes open polygons. To have closed polygons is
      highly important because just such polygons can be filled.
      ...
      parts of the polygon. Furthermore, partial data sets may be used as
      ...
      \textsf{no\_corner}, and \textsf{copy}. The 1st two are of type boolean.
      If these
      parameters are omitted, both are set to 0 (false) by default.
      ...
      newly joined longer and closed way. If the values of these keys differ than
      ...
      \subsubsection{Inherit Tags to Objects} \label{sec:act-inherit-tags}
...
\end{verbatim}
      ...
      if they do already exist.
         ...
         \subsubsection{Generating a Grid}\label{sec:act-grid}\label{sec:act-grid2} %FIXME: not finished

         \begin{verbatim}
   <action>   := ( 'grid:' | 'grid2:' ) <param> [ ';' <param> ]
   <param>    := <margin> | <ticksw> | <sticksw>
                 | <grid> | <ticks> | <sticks>
                 | <copyrgt> | <cmdline>
                 | <gridpts> | <polygon>

   <margin>   := 'margin' '=' <length>
   <ticksw>   := 'tickswidth' '=' <length>
   <sticksw>  := 'subtickswidth' '=' <length>
   <grid>     := 'grid' '=' grid spacing in arc minutes
   <ticks>    := 'ticks' '=' major axis spacing in arc minutes
   <sticks>   := 'subticks' '=' minor axis spacing in arc minutes
   <copyrgt>  := 'copyright' '=' <bool>
   <cmdline>  := 'cmdline' '=' <bool>
   <gridpts>  := 'gridpoints' '=' integer >= 2
   <polygon>  := 'polygon_window' '=' <bool>
         \end{verbatim}
      ...
      The boolean arguments \optv{copyright} and \optv{cmdline} do not generate
      these nodes.

      Each grid line can be generated with a number of intermediate points. The
      option \optv{gridpoints} sets the total number of grid points. It has to
      ...
      while the latter creates it in the main function at the first match.
      This may be of interest if some rules should be called before the grid is
      constructed.

      \subsubsection{Create a Ruler}\label{sec:act-ruler}\label{sec:act-ruler2}   %FIXME: not finished
...
%   \item {\sffamily ruler}
      ...
      \subsubsection{Text Translation}\label{sec:act-translate}

\begin{verbatim}
   <action>    := 'translate:' [ <param> [ ';' <param> ] ]
   <param>     := <keydef> | <iddef> | <newtagdef> | <coldef>
   <keydef>    := 'key' '=' <keyval> [ ';' <keydef ]
   <keyval>    := <string> | '/' <regex> '/'
   <iddef>     := 'id' '=' <objid>
...
\end{verbatim}
   ...
   Section \ref{sec:act-set-tags}). This is a rule node with an action. It has to have a node
   ...
   Have a look at Section \ref{sec:transexample} for a detailed example.
      ...
      \subsubsection{Translate Reference Point of Chart} \label{sec:act-transcoord}
...
\end{verbatim}
   ...
   \begin{itemize}
      ...
      \item {\sffamily bearings} \label{sec:act-bearings}

         This functions adds tags containing bearing information to the nodes
         of a way. Ways are series of nodes connected on after each other. If
         we look from one node to the next, the next one lies in a specific
         direction: this is the bearing and is set with the tag
         \textsf{smrender:bearing=<brg>} where \textsf{brg} is the angle
         between 0 and 360 degrees.

         Additionally the tag \textsf{smrender:coursedev=<dev>} is added. It
         contains an angle which states how much the bearing has changed in
         comparison to the previous segment of the way. \textsf{Dev} is an
         angle between -180 and 180 degrees.

         The 3rd parameter is \textsf{smrender:peakdir=<pd>} which contains an
         angle to wich the "peak" between segments (the middle node so to say)
         points at. This could be used e.g. for putting captions at this points
         in an appropriate angle to the way.

         All angles are expressed as compass bearings.

      \item {\sffamily boundingbox} \label{sec:act-boundingbox}

         This function calculates the bounding box of way and adds the tag
         \textsf{smrender:bbox=<lat0>,<lon0>,<lat1>,<lon1>} to the way. The
         coordinats \textsf{lat0} and \textsf{lon0} are the left lower
         (southwest) corner and \textsf{lat1} and \textsf{lon1} are the right
         upper (northeast) corner of the rectangle.

      \item {\sffamily check} \label{sec:act-check}

         This function does an integrity check on ways. This is it checks for
         subsequent duplicate nodes which will be removed from the list of
         references. If the final way has 0 nodes, the whole way will be
         removed from the data.

      \item {\sffamily clip} \label{sec:act-clip}
   ...
   \item {\sffamily dist\_median} (\textsc{experimental}) \label{sec:act-dist-median}
   ...
   \item {\sffamily fixbordernodes} \label{sec:act-fixbordernodes}
   ...
   \item {\sffamily global\_grid} \label{sec:act-global-grid}

      This function adds a global grid to the data, including the arctic
      circles.

      Parameters: \textsf{lat\_grid, lon\_grid, grid, gridpoints}

   \item {\sffamily ins\_eqdist} (\textsc{experimental}) \label{sec:act-ins-eqdist}
   ...
   \item {\sffamily mask} (\textsc{experimental})\label{sec:act-mask}
   ...
   \item {\sffamily poly\_area} \label{sec:act-poly-area}
   ...
   \item {\sffamily poly\_centroid} \label{sec:act-poly-centroid}
   ...
   \item {\sffamily poly\_len} \label{sec:act-poly-len}

      This function calculates the length of a polygon in nautical miles. It
      adds the tag \textsf{smrender:length=*} to the way.

   \item {\sffamily random} \label{sec:act-random}

      This function sets a tag with a random number.

\begin{verbatim}
   <action> := 'random:' ['key' '=' <string> ';' ] 
                         ['lo' '=' <num> ';'] 
                         ['hi' '=' <num> ';']
                         ['type' '=' <bool>]
\end{verbatim}

   The random number will be between \textsf{lo} and \textsf{hi}. If
   \textsf{type} is not set or set to \textsf{false}, the random number will be
   an integer number. Otherwise a random decimal number will be generated.

   The number is set in the tag \textsf{smrender:random=<num>} except the key
   is set differently with the parameter \textsf{key}.

   \item {\sffamily set\_ccw} and {\sffamily set\_cw}\label{sec:act-set-ccw}\label{sec:act-set-cw}

      Those functions set the direction of a closed way to either clockwise
      (``cw'') or counterclockwise (``ccw'').

   \item {\sffamily simplify}\label{sec:act-simplify}
   ...
   \item {\sffamily split}\label{sec:act-split}
   ...
   \item {\sffamily refine\_poly} (\textsc{deprecated})\label{sec:act-refine-poly}
   ...
   \item {\sffamily reverse\_way}\label{sec:act-reverse-way}

      This function always reverses a way independent of its current direction.

   \item {\sffamily virtclosed}\label{sec:act-virtclosed}
   ...
   \item {\sffamily wrapdetect}\label{sec:act-wrapdetect}
...
\end{verbatim}
   ...
   \item {\sffamily zeroway} (\textsc{experimental})\label{sec:act-zeroway}
...
\subsubsection{Output of OSM Data} \label{sec:output} \label{sec:act-out}
...
output it in JSON format as well and \textsf{format=js} it outputs the data
formated as Javascript file suitable to be included in a web page or node
program.
This action has one mandatory option which is
...
\begin{verbatim}
   ...
   <param>       := <file> | <format> | <renumber> | <jsonopts> | <precision>
...
The coordinates of nodes are output as decimal numbers with a precision of 7 by
default. This can be changed with parameter \textsf{precision}. It should be an
integer number between 0 and 20.

There are currently three different output formats.

\begin{itemize}
   \item \textbf{OSM:} This is the traditional XML/OSM format containing nodes,
      ways, and relations in that order. This files are well-formed and can be
      read in any other OSM software (such as e.g. Josm).

   \item \textbf{JSON:} This is a translation of the classic OSM format into
      JSON containing three arrays which hold all nodes, ways, and relations.

      The argument \textsf{compact=yes} modifies the format into much more
      dense version. In this case the ways contain not references to nodes (as
      they do in the original format) but directly the coordinates of the
      nodes. In contrast, nodes are only found in the array of nodes if they
      own at least one tag.

   \item \textbf{JS:}
      This format is almost the same as JSON but it is suitable to be directly
      included in Javascript either e.g. with a script tag from HTML or with
      \textsf{require()} in a node program.

\end{itemize}

\subsubsection{Executing Programs and Scripts} \label{sec:exec}\label{sec:act-exec}
...
\subsubsection{Calling Sub-Rules} \label{sec:act-sub}

The ruleset of \smrender{} allows to call a set of sub-rules. The purpose of this is to make the ruleset more simple and modular. Sub-rules can be nested and are re-entrant. That means that you can call a further subset from within a subset and that a subset can be called multiple times (like function calls in a programming language).

The subroutine call is implemented as a regular action with the following syntax.

\begin{verbatim}
   <action>   := 'cap:' <param> [ ';' <param> ]
   <param>    := <version>
   <version>  := 'version' '=' integer number
\end{verbatim} 

The option \optv{version} is mandatory. It specifies the version number of the group of rules to be called. All rules with this version number and the same object type (node, way, or relation) are then called in the order of their id ascendingly. These rules are then applied to the all objects which match the criteria defined in the sub-rule definition \emph{and} the sub-rules themselves. Have a look at the following example.

\begin{lstlisting}[language=XML]
<!-- call subrules for objects matching these tags -->
<way> <!-- rule #1 -->
   <tag k='natural' v='coastline'/>
   <tag k='generator' v='smrender'/>
   <tag k='_action_' v='sub:version=65536'/>
</way>

<!-- subrules to be called -->
<way version="65536"> <!-- rule #2 -->
   <tag k='_action_'
   v='draw:color=navajowhite;directional=1;ignore_open=1'/>
</way>

<!-- nested subroutine call -->
<way version="65536"> <!-- rule #3 -->
   <tag k='_action_' v='sub:version=65537'/>
</way>

<!-- next level subroutines -->
<way version="65537"> <!-- rule #4 -->
   <tag k='_action_' v='draw:bcolor=black'/>
</way>

<!-- repeated subroutine call-->
<way version="2"> <!-- rule #5 -->
...
This example contains 5 rules in 4 groups. The 1st one has no version attribute thus 1 is assumed. The 2nd and 3rd one are in group \#65536, the 4th is in group \#65537 and the 5th is in group \#2.

The 1st rule to be executed is the 1st at the beginning because it has the lowest version (group) number and the lowest id -- obviously because it is the only element in the group. The 2nd, 3rd, and 4th rule are ignored by the regular rule processor because their version number is $\geq$ 65536 (see Section \ref{ref:ruleorder}). The 5th rule is executed next after the 1st one.

The execution is then done as follows. The 1st rule is executed and applied to all ways which have the tags \textsf{natural=coastline} and \textsf{generator=smrender}. Since the action is \textsf{sub(version=65536)}, "applying" means executing all rules from the given subset. This subset contains 2 rules: the 2nd and the 3rd from the ruleset from above. The 2nd is executed 1st because it has the lower id. This is because both rules have no explicitly defined id so they are numbered with an internal number ascendingly as they appear. The 3rd is executed directly after \#2. Since this rule is again a subroutine call to the group \#65537, rule \#4 is executed.
...
\begin{itemize}
   \item {\sffamily diff} (\textsc{experimental}) \label{sec:act-diff}
   ...
   \item {\sffamily disable} \label{sec:act-disable}
   ...
   \item {\sffamily enable\_rule} and {\sffamily disable\_rule} \label{sec:act-disable-rule}\label{sec:act-enable-rule}
   ...
   \item {\sffamily exit} \label{sec:act-exit}
   ...
   \item {\sffamily incomplete} (\textsc{experimental}) \label{sec:act-incomplete}
   ...
   \item {\sffamily neighbortile} (\textsc{experimental}) \label{sec:act-neighbortile}
...
\emph{INT1/Chart No. 1} Section P and P.16 in particular.\footnote{\url{http://www.nauticalcharts.noaa.gov/mcd/chart1/ChartNo1.pdf}}

\begin{verbatim}
   <action> := 'pchar@libsmfilter.so:' [ <param> [ ';' <param> ]]
   <param>  := <lang> | <compat> | <colkey>
   <lang>   := 'lang' '=' ( 'de' | 'en' | 'gr' | 'hr' )
   <compat> := 'compat' '=' <bool>
   <colkey> := 'colkey' '=' regular expression
\end{verbatim}
...
supports the optional parameter \optv{lang} having \textsf{'en'} as default.
It depends on the compatibility setting of \textsf{'compat'} how it will behave.

By default it is set to \textsf{compat=yes} for backwards compatibility. In this case it will 1st translate
the tags into the chosen language and then format it according to the local \emph{INT1}.

Please note that this mode is deprecated since adding new languages would require to change the code the new suggested variant is to set \textsf{compat=no}. In this case it formats the string according the chosen language's \emph{INT1} but it does not translate the light characters and colors. This must be done beforehand by using the action \textsf{translate()} (see \ref{sec:act-translate}).

The following table shows language examples according to \emph{INT-1, IP 51}:\\
...
\begin{verbatim}
   <action> := 'compass@libsmfilter.so:' <param> [ ';' <param> ]
   <param>  := <var> | <radius> | <ticks> | <stldef>
   <var>    := 'variation' '=' decimal value
   <radius> := 'radius' '=' decimal value
   <ticks>  := 'ticks' '=' integer value
   <stldef> := 'style' '=' ( 'int1' | 'int1var' )
\end{verbatim}
