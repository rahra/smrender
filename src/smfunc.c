...
int act_split_main(smrule_t *r, osm_node_t *n)
{
   ...
   for (; *optr != NULL; optr++)
   {
      ...
      {
         // check if way was generated by Smrender
         gentag = match_attr(*optr, "generator", "smrender") < 0;
         // alloc new way
         w = malloc_way((*optr)->tag_cnt + 1 + gentag, refs);
         osm_way_default(w);
         // copy all tags
         memcpy(&w->obj.otag[gentag], (*optr)->otag, (*optr)->tag_cnt * sizeof(*(*optr)->otag));
