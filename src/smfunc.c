...
int act_split_main(smrule_t *r, osm_node_t *n)
{
   ...
   for (; *optr != NULL; optr++)
   {
      ...
      if ((refs = ((osm_way_t*) (*optr))->ref_cnt - i + 1 - rs) > 1)
      {
         ...
         osm_way_default(w);
         // copy all tags (intentionally, generator=smrender is overwritten
         memcpy(&w->obj.otag[0], (*optr)->otag, (*optr)->tag_cnt * sizeof(*(*optr)->otag));
...
int act_fixbordernodes_ini(smrule_t *r)
{
   double dist;
   ...
   {
      dist = 1;
      log_debug("parameter 'dist' missing, setting to default value %.2f''", dist);
   }
   dist /= 3600;
   ...
   log_debug("dist = %f deg (%.2f'')", dist, dist * 3600);
   *((double*) r->data) = dist;
   return 0;
}
 ...
 */
int act_fixbordernodes_main(smrule_t *r, osm_node_t *n)
{
   double dist = *((double*) r->data);
...
int act_fixbordernodes_fini(smrule_t *r)
{
   free(r->data);
   r->data = NULL;
   return 0;
